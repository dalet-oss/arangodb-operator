...
import (
	...
	"github.com/dalet-oss/arangodb-operator/pkg/api"
	...
	"github.com/dalet-oss/arangodb-operator/pkg/deployment/agency/cache"
	...
	"github.com/dalet-oss/arangodb-operator/pkg/operator"
	...
	utilsError "github.com/dalet-oss/arangodb-operator/pkg/util/errors"
...
func newOperatorConfigAndDeps(id, namespace, name string) (operator.Config, operator.Dependencies, error) {
    ...
    if scope.IsCluster() {
        watchNamespace = meta.NamespaceAll
    } else {
        watchNamespace = namespace
    }

    eventRecorder := createRecorder(client.Kubernetes(), name, watchNamespace)
